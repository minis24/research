...
public class ListTransform {
	...
	public static void main(String[] args) {
		...
		// map()메서드는 스트림의 각 엘리먼트를 처리하여 실행결과를 취합하여 결과 컬렉션으로 리턴한다. 
		// map()메서드는 입력 엘리먼트의 수와 결과 엘리먼트의 수가 동일하다는 것을 보장한다.
		friends.stream()
			.map(name -> name.toUpperCase())
			.forEach(name -> System.out.print(name + " "));
		
		
		//stream()메서드는 스트림 인스턴스에 대한 컬렉션을 래핑한다.
		//==> 람다표현식을 사용하면, 명시적 변경이 없으며, 간결하다.
		//==> 더이상 비어있는 컬렉션의 초기화 작업이나, 가비지 변수가 필요하지 않다.
